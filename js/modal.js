...
function hideCPopup() {
  document.querySelector(".cpopup").classList.remove("popup_active");
  document.querySelector(".popup-overlay").style.display = "none";
  document.body.style.overflowY = "scroll";
}
document.getElementById("edit-btn").addEventListener("click", showCPopup);
document.querySelector(".cclose-btn").addEventListener("click", hideCPopup);
document.querySelector(".popup-overlay").addEventListener("click", hideCPopup);
document.addEventListener("keydown", function (e) {
  if (e.key === "Escape") {
    hideCPopup();
  }
